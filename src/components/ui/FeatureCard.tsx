...
export function FeatureCard({
  ...
  return (
    <div className={cn("relative group pt-8 md:pt-14 h-full", className)}>
      ...
      <div className={cn(
        "absolute bottom-0 right-0 w-[95%] h-[80px] md:h-[120px] bg-gradient-to-r rounded-[24px] md:rounded-[32px] rotate-[6deg] -translate-x-1 translate-y-4 z-0 opacity-70 transition-all duration-700 group-hover:rotate-[8deg] group-hover:translate-y-6 group-hover:opacity-100",
        gradientFrom,
        gradientTo
      )}></div>
      ...
      <div className="relative w-full h-full transition-all duration-500 group-hover:-translate-y-2 z-10">
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-[#1e294b] via-[#141d3d] to-[#0f172a] rounded-[24px] md:rounded-[32px] shadow-[0_20px_50px_rgba(0,0,0,0.6)] border border-white/10 overflow-hidden group-hover:shadow-[0_30px_60px_rgba(0,0,0,0.8)] transition-all duration-500">
          
          {/* Premium Fold Effect (Top Right Corner) */}
          <div className="absolute top-0 right-0 w-10 h-10 md:w-16 md:h-16 pointer-events-none">
            {/* The Crease Shadow */}
            <div className="absolute top-0 right-0 w-full h-full bg-gradient-to-bl from-black/40 to-transparent z-20"></div>
            {/* The Fold Highlight */}
            <div className="absolute top-0 right-0 w-0 h-0 border-t-[20px] md:border-t-[32px] border-r-[20px] md:border-r-[32px] border-t-white/10 border-r-transparent z-30"></div>
          </div>
        ...
        {/* Content layer */}
        <div className="relative px-5 md:px-8 pt-10 md:pt-16 pb-6 md:pb-12 flex flex-col items-center text-center h-full">
          ...
          {/* Title */}
          <h3 className="text-white text-lg md:text-2xl font-bold tracking-tight z-20 relative drop-shadow-md group-hover:text-white transition-colors">
            {title}
          </h3>

          {/* Description */}
          <p className="text-[#cfd6ec] text-[10px] md:text-sm leading-relaxed mt-3 md:mt-5 flex-1 z-20 relative font-medium">
            {description}
          </p>

          {/* Learn More Link - Enhanced for engaging UI */}
          {href && (
            <div className="mt-5 md:mt-8 pt-4 md:pt-6 w-full flex justify-center z-20 relative">
